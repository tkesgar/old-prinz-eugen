<template>
  <b-modal
    ...
    @hidden="reset"
  >
    <b-form id="CreateCharacterModal_form" @submit.prevent="submitForm">
      <b-form-group
        label="Nama karakter"
...
export default {
  ...
  watch: {
    fullName (newFullName) {
      const autoNickName = _toNickName(newFullName)
      const autoName = _toName(newFullName)

      if (_shouldAutoChange(this.nickName, autoNickName)) {
        this.nickName = autoNickName
      }

      if (_shouldAutoChange(this.name, autoName)) {
        this.name = autoName
      }
    }
  }
...
function _shouldAutoChange (val, autoVal) {
  return val.trim().length === 0 || autoVal.startsWith(val)
}

function _toNickName (str) {
  return str.trim().split(/\s+/)[0]
}

function _toName (str) {
